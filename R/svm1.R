...
#'
svm1=function(yflag=F,traindata,testdata,ytrain,ytest){
  if (yflag==T){
    time=system.time({svt=svm(as.matrix(traindata),as.factor(ytrain),type="C-classification",scale = FALSE,kernel = "radial")})
